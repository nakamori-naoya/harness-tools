...
| `tools/lint-consumer-contract.py` | 消費側の文書・script・設定に外部依存の内部名（内部 skill 名、工程 id、保存モード名、CLI 引数）が漏れていないかの静的 lint | `HARNESS_PLUGIN_DEV_ROOTS=<契約ID→package root の JSON> python3 ../harness-tools/tools/lint-consumer-contract.py --repo <repo> --runtime claude\|codex [--json]` |
