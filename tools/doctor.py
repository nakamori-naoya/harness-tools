...
"""Read-only CLI, publication, dependency and configuration diagnostics.
  ...
  doctor.py --repository <plugin repository の絶対path> [--repo <対象project>] [--distribution-only]

CLI の有無、両 runtime の公開入口、全公開入口 / 内部 skill の外部依存を、兄弟 checkout
（<repository>/../<marketplace>-plugins）の実配布物に対して隣の resolve-dependency.py で解決する。
JSON（checks[]）を標準出力へ返し、1 つでも ok=false なら終了 code 1。repository には何も書かない。
"""
import argparse
import json
import os
...
def main():
    ...
    p.add_argument('--repository', required=True, help='diagnosed plugin repository (absolute path)')
