...
package root だけの manifest、公開 playbook の CONTRACT.md）で作る。tool そのものの検査は
合成 fixture だけで閉じる。--repository を渡したときだけ、その repository の CI workflow、
公開入口、doctor の読み取り専用性、write-doc の template 対応も検査する（無ければ skip として数える）。
"""
...
RESOLVER = TOOLS/'resolve-dependency.py'
REPOSITORY = None
...
class Hardening(unittest.TestCase):
    ...
    def _repository(self):
        if REPOSITORY is None:
            self.skipTest('--repository が無い（tool の自己検査だけを実行）')
        return REPOSITORY
    def test_doctor_rejects_external_skill_and_symlinked_script_before_access(self):
        ...
        result=self.call('python3',TOOLS/'doctor.py','--repository',repo,'--distribution-only')
        ...
        result=self.call('python3',TOOLS/'doctor.py','--repository',repo,'--repo',self.base)
        self.assertEqual(result.returncode,1)
        self.assertFalse(marker.exists())
        self.assertEqual(len(json.loads(result.stdout)['checks']),1)
    def test_functional_ci_actions_are_pinned(self):
        workflow = (self._repository()/'.github/workflows/validate.yml').read_text()
    ...
    def test_doctor_does_not_mutate_repository(self):
        ...
        r=self.call('python3',TOOLS/'doctor.py','--repository',root,'--distribution-only')
    ...
    def test_release_updates_all_matching_declarations_only(self):
        ...
        r=self.call('python3',TOOLS/'release.py','--repo',repo,'--plugin','fixture','--version','2.0.0','--notes','change','--breaking','yes','--migration','new run','--checks',checks,'--apply')
        ...
        r=self.call('python3',TOOLS/'release.py','--repo',repo,'--plugin','fixture','--version','3.0.0','--notes','change','--breaking','yes','--migration','new run','--checks',checks,'--apply')
        self.assertNotEqual(r.returncode,0)
        self.assertEqual(before,{str(p):p.read_bytes() for p in repo.rglob('*.json')})
        for version in ['1.0.0-alpha..1','1.0.0-.','1.0.0-01']:
            result=self.call('python3',TOOLS/'release.py','--repo',repo,'--plugin','fixture','--version',version,'--notes','x','--breaking','x','--migration','x','--checks',checks)
    ...
    def test_semantic_runner_records_assessment_but_only_operational_errors_fail(self):
        ...
        common=['python3',TOOLS/'evaluate-skills.py','--fixtures',fixtures,'--model-command',command,'--judge-command',command,'--model','generator','--judge-model','judge','--output',out]
    ...
    def test_removed_resolver_commands_are_rejected(self):
        """--resolve-inputs / --explain-config / --check-input は第2回で呼び手が消えたので受け付けない。"""
        for option in ('--resolve-inputs', '--explain-config', '--check-input'):
            result = self.call('python3', RESOLVER, option, input='{}')
            self.assertEqual(result.returncode, 2, option)
            self.assertIn('--plugin-root', result.stderr, option)
    ...
    def test_doctor_resolves_playbooks_against_sibling_checkouts(self):
        ...
        def diagnose(extra=None):
            result = subprocess.run(
                ['python3', str(TOOLS/'doctor.py'), '--repository', str(repo), '--repo', str(repo)],
    ...
    def test_consumer_lint_flags_provider_internal_names(self):
        ...
        entry = self._consumer('  - {id: work, playbook: write-doc, purpose: fixture, provides: [x]}\n', [('write-doc', 'write-doc')])
        repo = self.base/'consumer'
        clean = subprocess.run(['python3', str(TOOLS/'lint-consumer-contract.py'), '--repo', str(repo), '--runtime', 'codex', '--json'],
        ...
        linted = subprocess.run(['python3', str(TOOLS/'lint-consumer-contract.py'), '--repo', str(repo), '--runtime', 'codex', '--json'],
                                text=True, capture_output=True, env=environment, timeout=120)
